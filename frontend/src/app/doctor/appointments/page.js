    ...
    const fetchPatientDetails = async () => {
      try {
        const response = await axios.get(
          `${API_URL}/api/patient-profiles/${patientId}`,
        );
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-gray-200 p-6">
      <div className="max-w-5xl mx-auto bg-gray-800 shadow-md rounded-lg p-8">
        <h1 className="text-4xl font-extrabold text-gray-100 mb-6 border-b border-gray-700 pb-4">
          Patient Details
        </h1>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            ...
            <p className="text-lg">
              <span className="font-semibold text-gray-300">Blood Group:</span>{" "}
              {patientDetails.bloodGroup}
            </p>
            <p className="text-lg">
              <span className="font-semibold text-gray-300">Allergies:</span>{" "}
          ...
          <div className="space-y-4">
            ...
            <p className="text-lg">
              <span className="font-semibold text-gray-300">Relation:</span>{" "}
            ...
            {patientDetails.diagnosticReports.map((report, index) => (
              <li key={index} className="hover:text-blue-400">
                ...
                <span className="text-gray-400">
                  (Uploaded at: {new Date(report.uploadedAt).toLocaleString()})
                </span>
              </li>
            ))}
          </ul>
          ...
          <ul className="list-disc list-inside space-y-2">
            {patientDetails.medicalRecords.map((record, index) => (
              <li key={index} className="hover:text-blue-400">
                ...
                <span className="text-gray-400">
                  (Uploaded at: {new Date(record.uploadedAt).toLocaleString()})
                </span>
              </li>
            ))}
          </ul>
